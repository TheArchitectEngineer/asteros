...
xfm.install.script::	bin/xfm.install.cpp
	$(RM) $@
	 $(CPP)  -DLIBDIR=$(XFMLIBDIR) <bin/xfm.install.cpp | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' -e '/^XCOMM$$/s//#/' -e '/^XCOMM[^a-zA-Z0-9_]/s/^XCOMM/#/' >$@
	chmod a+x $@
...
install:: xfm.install.script
	@if [ -d $(DESTDIR)$(DESTDIR)$(BINDIR) ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(DESTDIR)$(DESTDIR)$(BINDIR)); fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTBINFLAGS) xfm.install.script $(DESTDIR)$(DESTDIR)$(BINDIR)/xfm.install

Xfm.ad::  lib/Xfm.cpp src/FmVersion.h
	$(RM) $@
	 $(CPP)  -DLIBDIR=$(XFMLIBDIR) -DXAW3D $(ENH_DEFINES) <lib/Xfm.cpp | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' -e '/^XCOMM$$/s//#/' -e '/^XCOMM[^a-zA-Z0-9_]/s/^XCOMM/#/' >$@
...
install:: Xfm.ad
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) Xfm.ad $(DESTDIR)$(XAPPLOADDIR)/Xfm

install::
	@if [ -d $(XFMLIBDIR)/bitmaps ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(XFMLIBDIR)/bitmaps); fi
	$(INSTALL) -c $(INSTDATFLAGS) lib/bitmaps/?* $(XFMLIBDIR)/bitmaps
	@if [ -d $(XFMLIBDIR)/pixmaps ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(XFMLIBDIR)/pixmaps); fi
	$(INSTALL) -c $(INSTDATFLAGS) lib/pixmaps/?* $(XFMLIBDIR)/pixmaps

install::
	@if [ -d $(XFMLIBDIR)/icons ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(XFMLIBDIR)/icons); fi
	$(INSTALL) -c $(INSTDATFLAGS) contrib/3dicons/icons/?* $(XFMLIBDIR)/icons
	@if [ -d $(XFMLIBDIR)/dot.xfm ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(XFMLIBDIR)/dot.xfm); fi
