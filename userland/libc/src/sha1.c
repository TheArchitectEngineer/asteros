/* SHA-1, FIPS 180-1. Public-domain reference implementation (original
 * by Steve Reid <steve@edmweb.com>, the same code real BSD libmd and
 * Darwin's own libc trace back to) -- ground-truthed against the FIPS
 * 180-1 spec, not adapted from any Apple-licensed source. Needed for
 * the X11 milestone; see include/sha1.h. */
#include <sha1.h>
#include <string.h>
...
SHA1Update(SHA1_CTX *context, const unsigned char *data, size_t len)
{
	...
	context->count[0] += (uint32_t)(len << 3);
